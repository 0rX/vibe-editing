...
def _tile_cmd(chunk, out, side_cfg, res, detw):
    ...
    cmd = [PY, str(QA_REFRAME), str(chunk), str(out),
...
def run(work_dir, config, inputs, inputs_meta, project, manifest, out_path):
    ...
    if config.get("per_segment"):
        ...
        for i, s in enumerate(segs):
            ...
            if i in split_idxs:
                ...
                ff([PY, str(MAKE_SPLIT), "--speaker", str(top), "--guest", str(bot),
                ...
                scmd = [PY, str(QA_REFRAME), str(chunk), str(seg_clip),
    ...
    cmd = [PY, str(QA_REFRAME), str(cut_out), str(full_out),
