...
def main():
    ...
    def build_masters(role, cams_set, zoom, eye, roi, tag, lock_y=True, panel_mode=False, tfh=0.34, tfy=0.34):
        ...
        if legacy:
            ...
            for j, (r0, r1) in enumerate(rngs):
                ...
                if not (os.path.exists(mp) and os.path.getsize(mp) > 1e6):
                    ...
                    _rcmd = [PY, REFRAME, tmp, mp, "--res", ("4k" if OW >= 2160 else "1080"), "--zoom", str(zoom),
        ...
        if not (os.path.exists(mp) and os.path.getsize(mp) > 1e6):
            ...
            if panel_mode:
                ...
                cmd = [PY, GUEST_PANEL, concat_in, mp, "--target-face-h", str(tfh), "--target-face-y", str(tfy),
                       "--roi", roi, "--panel-w", str(OW), "--panel-h", str(OH // 2), "--fps", FPS]
            else:
                cmd = [PY, REFRAME, concat_in, mp, "--res", ("4k" if OW >= 2160 else "1080"),
    ...
    if os.path.exists(LAYOUT_ANALYZE):
        _la = subprocess.run([PY, LAYOUT_ANALYZE, base, f"{work}/cc_layout.json",
    ...
    gen_cmd = [PY, GEN_SPICE, f"{work}/w_spice.json", "--preset", a.preset,
